...
def SystVariation(SystName, Campaign = "MC16a", UseOddForTrain = True ):
    ReadNtuples = False
    #UseOddForTrain = True      # True means test on Even    
    #DirectoryName = "Log_MainOutput"
    DirectoryName = "TestOutput"
    epochs = 1 #100 for default
    
    DoData = False
    
    #Test
    path = "/rdsk/dat24/atlas/kkreul/combination/A++/v02_v34lj_TEST_MC16a/sgtop_tchan_signal/AtlSgTop_tChannelAnalysis/MemDisc/2Jets/lnu/nominal/"
    ...
    CampaignName = "Even/" + Campaign if UseOddForTrain else "Odd/" + Campaign
    
   
    
    
    if (ReadNtuples):
        Reader = DataReader(path, DoData, Campaign)
    ...
    Df = Preproc.ScaleToXSection(Df,Campaign,DoData)
    ...
    #X_train_NoWeight = Preproc.ScaleToXSection(X_train, Campaign, DoData)
    #X_test_NoWeight = Preproc.ScaleToXSection(X_train, Campaign, DoData)
    
    
    X_train_scaled, X_test_scaled, Weight_train, Weight_test = Preproc.Scale(X_train, X_test)
    
    if SystName == "nominal" and not DoData:
        # Bes setup so far in terms of overfitting: 1 layer, 50 nodes, L2 kernel regu 1e-3
        ...
        Model.save("/users/eeh/kkreul/MachineLearning/Env2/schanmachinelearning/"+DirectoryName+"/"+CampaignName+"/NominalModel")
    
    else:
    
        Model = keras.models.load_model('/users/eeh/kkreul/MachineLearning/Env2/schanmachinelearning/'+DirectoryName+'/' +CampaignName+'/NominalModel')
    ...
    SystName = SystName if not DoData else "nominalData"
    
    Plot = Plotter(Directory  = DirectoryName+"/"+CampaignName)
    ...
    Writer.WriteData(DirectoryName+"/"+CampaignName)
...
if __name__ == "__main__": 
    ...
    Syst = sys.argv[1]
    Campaign = sys.argv[2]
    TrainOnOdd = True if sys.argv[3] == "Odd" else False
    print("Running for Syst: " + Syst)
    sys.exit(SystVariation(Syst, Campaign, TrainOnOdd))
    #if Syst == "nominal":
    #    sys.exit(nominal())
    #else: 
    ...
    print(Df.columns)
    print(X_test["Lead_jet_pt"])
    
    Plot = Plotter(Directory  = DirectoryName+"/"+Campaign)
    Plot.MakePlot("NeuralNetOutput", X_test, SystName = SystName)
    #Plot.MakePlot("NeuralNetOutput", X_test, isDens = False,  SystName = SystName)
    Plot.MakePlot("llh_sChannel2j", Df, SystName = SystName)
    Plot.MakePlot("Lead_jet_pt", X_test, SystName = SystName)
